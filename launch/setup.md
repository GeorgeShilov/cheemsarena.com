...
- for this project, the domain is `cheemsarena.com`
...
```text
https://web.archive.org/web/20160328162852/cheemsarena.com/
```
...
- Railway project: `cheemsarena.com`
- Railway service: `cheemsarena.com`
- Railway temporary domain: `https://codexworkplacecom-production.up.railway.app`
- Custom domain: `cheemsarena.com`
...
```bash
railway init -n cheemsarena.com -w <workspace-id> --json
railway add --service cheemsarena.com --json
railway up --service cheemsarena.com --detach --verbose
railway domain --service cheemsarena.com --json
railway variable set SITE_URL=https://cheemsarena.com --service cheemsarena.com --environment production --skip-deploys
```

If `railway domain cheemsarena.com ...` returns `Unauthorized` even after `railway login`, use the Railway dashboard or Railway GraphQL API to create the custom domain. In this run, the CLI was intermittently authenticated for `status` but unauthorized for `domain`.
...
Railway gave these records for `cheemsarena.com`:
...
Zone > DNS > Edit
Zone > Zone > Read
Zone Resources: Include > Specific zone > cheemsarena.com
```
...
const zoneName = "cheemsarena.com";
...
```bash
curl -s "https://cloudflare-dns.com/dns-query?name=cheemsarena.com&type=A" \
  -H "accept: application/dns-json"

curl -s "https://cloudflare-dns.com/dns-query?name=_railway-verify.cheemsarena.com&type=TXT" \
  -H "accept: application/dns-json"
```
